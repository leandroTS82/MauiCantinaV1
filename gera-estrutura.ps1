...
$foldersToList = @("Data", "Models", "Popups","Services", "Views", "ViewModels")
...
foreach ($folder in $foldersToList) {
    ...
    if ($folder -eq "Services") {
        ...
        $indentServices = "│   "
        Add-Content $outputFile "$indentServices├── Externals        (Serviços externos: Firebase, WhatsApp, CSV, XLSX)"
        $externalsPath = Join-Path $fullPath "Externals"
        if (Test-Path $externalsPath) {
            List-FolderContent -path $externalsPath -indentLevel 2
        }
        Add-Content $outputFile "$indentServices└── Internals        (Serviços internos: Pedidos, Produtos, Configurações)"
